...
const KPICardsSkeleton = () => (
  <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 text-left">
    {Array.from({ length: 7 }).map((_, idx) => (
      <div 
        key={idx} 
        className="bg-zinc-900/40 border border-zinc-800/80 rounded-2xl p-5 relative overflow-hidden backdrop-blur-sm shadow-md animate-pulse"
      >
        <div className="flex items-center justify-between">
          <div className="h-3.5 bg-zinc-800 rounded-md w-24" />
          <div className="h-4.5 w-4.5 bg-zinc-850 rounded-full" />
        </div>
        <div className="mt-4 flex items-baseline justify-between">
          <div className="h-7 bg-zinc-800 rounded-md w-20" />
          <div className="h-4 bg-zinc-850 rounded-md w-10 border border-zinc-800" />
        </div>
      </div>
    ))}
  </div>
...
const MetricsChartSkeleton = () => (
  <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-3xl p-6 space-y-6 backdrop-blur-sm shadow-md animate-pulse text-left">
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div className="space-y-2">
        <div className="h-5 bg-zinc-800 rounded-md w-48" />
        <div className="h-3 bg-zinc-850 rounded-md w-64" />
      </div>
      <div className="flex items-center gap-2 bg-zinc-950 p-1 border border-zinc-850 rounded-2xl w-full sm:w-auto overflow-hidden">
        {Array.from({ length: 4 }).map((_, idx) => (
          <div key={idx} className="h-7 bg-zinc-900 rounded-xl w-24" />
        ))}
      </div>
    </div>

    <div className="h-72 w-full bg-zinc-950/40 rounded-2xl border border-zinc-850 p-6 flex flex-col justify-between relative overflow-hidden">
      <div className="absolute inset-0 p-6 flex flex-col justify-between pointer-events-none opacity-40">
        {Array.from({ length: 5 }).map((_, idx) => (
          <div key={idx} className="border-b border-zinc-800/60 w-full h-0" />
        ))}
      </div>
      ...
      <div className="flex justify-between items-end mt-auto pt-4 border-t border-zinc-800/20">
        {Array.from({ length: 6 }).map((_, idx) => (
          <div key={idx} className="h-3 bg-zinc-900 rounded-md w-10" />
        ))}
      </div>
    </div>
  </div>
...
const TopPostsSkeleton = ({ activePlatform }) => (
  <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-3xl p-6 space-y-4 backdrop-blur-sm shadow-md animate-pulse text-left">
    <div className="flex items-center justify-between flex-wrap gap-3">
      <div className="h-5 bg-zinc-800 rounded-md w-48" />
      <div className="h-6 bg-zinc-850 rounded-full w-28" />
    </div>
            ...
            <tr key={idx}>
              <td className="py-4 pl-2">
                <div className="space-y-1.5">
                  ...
                  <div className="h-2.5 bg-zinc-850 rounded-md w-2/3" />
                </div>
              </td>
              {activePlatform === 'all' && (
                <td className="py-4">
                  <div className="h-5 bg-zinc-850 rounded-full w-16" />
                </td>
              )}
              <td className="py-4 text-center">
                <div className="h-4 bg-zinc-850 rounded-md w-8 mx-auto" />
              </td>
              <td className="py-4 text-center">
                <div className="h-4 bg-zinc-850 rounded-md w-8 mx-auto" />
              </td>
              ...
              <td className="py-4 text-center">
                <div className="h-4 bg-zinc-800 rounded-md w-10 mx-auto" />
              </td>
              <td className="py-4 text-right pr-2">
                <div className="h-4 bg-zinc-800 rounded-md w-12 ml-auto" />
              </td>
            </tr>
          ))}
        </tbody>
  ...
  <div className="space-y-5 text-left animate-pulse">
    <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-3xl p-6 backdrop-blur-sm shadow-lg">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3.5">
          ...
          <div className="space-y-1.5">
            <div className="h-4.5 bg-zinc-800 rounded-md w-32" />
            <div className="h-3 bg-zinc-850 rounded-md w-60" />
          </div>
        </div>
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="h-8 bg-zinc-850 rounded-xl w-24" />
          <div className="h-8 bg-zinc-850 rounded-xl w-16" />
        </div>
      </div>
    </div>
    ...
    <div className="space-y-3">
      <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-2xl p-5 space-y-2.5">
        <div className="h-3.5 bg-zinc-800 rounded-md w-36" />
        <div className="h-3 bg-zinc-850 rounded-md w-full" />
        <div className="h-3 bg-zinc-850 rounded-md w-4/5" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
        {Array.from({ length: 4 }).map((_, idx) => (
          <div key={idx} className="bg-zinc-900/40 border border-zinc-800/80 rounded-2xl p-5 space-y-3">
            <div className="h-4 bg-zinc-800 rounded-md w-1/3" />
            <div className="space-y-2">
              <div className="h-3 bg-zinc-850 rounded-md w-full" />
              <div className="h-3 bg-zinc-850 rounded-md w-5/6" />
            </div>
          </div>
        ))}
      </div>
      ...
      <AnimatePresence mode="wait">
        {loading ? (
          ...
          >
            ...
            <div className="space-y-2">
              <h2 className="text-xl font-bold text-zinc-100">No Analytics Data Available</h2>
            ...
            <div className="bg-zinc-900/40 border border-zinc-800/80 rounded-3xl p-6 space-y-6 backdrop-blur-sm shadow-md">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 text-left">
                <div>
                  ...
                  <p className="text-xs text-zinc-550 dark:text-zinc-500 mt-0.5">Performance tracking timeline for the past {activeDays} days</p>
                </div>
                      ...
                      <button
                        ...
                        className={`flex items-center space-x-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all cursor-pointer shrink-0 ${
                          isActive 
                            ...
                            : 'text-zinc-550 hover:text-zinc-300 dark:text-zinc-500 dark:hover:text-zinc-350'
                        }`}
                      >
              ...
              {postsLoading ? (
                ...
                <div className="w-full overflow-x-auto">
                  <table className="w-full min-w-[640px] text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-zinc-850 text-sm text-zinc-350">
                      {topPosts.filter(p => activePlatform === 'all' || p.platform === activePlatform).map((p) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <td className="py-4 text-center text-zinc-405 dark:text-zinc-400 font-bold">{formatMetric(p.likes)}</td>
                            <td className="py-4 text-center text-zinc-405 dark:text-zinc-400 font-bold">{formatMetric(p.comments)}</td>
                            <td className="py-4 text-center text-zinc-405 dark:text-zinc-400 font-bold">{formatMetric(p.shares)}</td>
                ...
                <div className="flex flex-col items-center justify-center py-12 space-y-3 text-center">
                  <div className="w-12 h-12 rounded-2xl bg-zinc-800/60 flex items-center justify-center">
                    <Sparkle className="h-6 w-6 text-zinc-650 dark:text-zinc-600" />
                  </div>
                  ...
                  <p className="text-xs text-zinc-650 dark:text-zinc-600 max-w-xs">
