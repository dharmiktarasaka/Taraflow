...
const PriorityBadge = ({ priority }) => {
  const map = {
    high:   { bg: 'bg-rose-500/15',   text: 'text-rose-400',   border: 'border-rose-500/30',   label: 'HIGH' },
    medium: { bg: 'bg-amber-500/15',  text: 'text-amber-400',  border: 'border-amber-500/30',  label: 'MED' },
...
const PlatformBadge = ({ platform }) => {
  const map = {
    linkedin:  { bg: 'bg-blue-500/10',  text: 'text-blue-400' },
    instagram: { bg: 'bg-pink-500/10',  text: 'text-pink-400' },
    facebook:  { bg: 'bg-blue-600/10',  text: 'text-blue-300' },
    threads:   { bg: 'bg-zinc-700/40',  text: 'text-zinc-300' }
  };
...
// ─── Section Card ────────────────────────────────────────────────────────────
const SectionCard = ({ icon: Icon, title, iconColor = 'text-indigo-400', children, defaultOpen = true }) => {
  const [open, setOpen] = useState(defaultOpen);
  return (
    <div className="bg-zinc-900/50 border border-zinc-800/70 rounded-2xl overflow-hidden backdrop-blur-sm">
      <button
        onClick={() => setOpen(o => !o)}
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="px-5 pb-5 pt-1 border-t border-zinc-800/50 text-left">{children}</div>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
      ...
      <AnimatePresence>
        {showPanel && (
          <motion.div
            ...
            className="absolute right-0 top-7 z-50 w-80 bg-zinc-900 border border-zinc-700 rounded-2xl shadow-2xl p-4 space-y-4 text-left"
          >
            {/* Header */}
            <div className="flex items-center gap-2">
              <Shield className="h-4 w-4 text-indigo-400" />
              <span className="text-sm font-bold text-white">AI Learning & Privacy</span>
            </div>

            {/* Info block */}
            <div className="bg-zinc-800/60 rounded-xl p-3 text-xs text-zinc-400 space-y-1.5">
              <p className="flex items-start gap-1.5"><CheckCircle2 className="h-3 w-3 text-emerald-400 mt-0.5 shrink-0" /> Only aggregated performance stats are stored — never raw post content or personal data.</p>
              <p className="flex items-start gap-1.5"><CheckCircle2 className="h-3 w-3 text-emerald-400 mt-0.5 shrink-0" /> Your data is isolated from all other users.</p>
              <p className="flex items-start gap-1.5"><CheckCircle2 className="h-3 w-3 text-emerald-400 mt-0.5 shrink-0" /> You can disable learning or delete all data at any time.</p>
            </div>

            {/* Toggle */}
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-bold text-zinc-200">AI Continuous Learning</p>
            ...
            <button
              onClick={onDelete}
              disabled={deleting}
              className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-xl text-xs font-semibold text-rose-400 border border-rose-500/20 bg-rose-500/5 hover:bg-rose-500/10 transition-colors disabled:opacity-40"
            >
  ...
  return (
    <div className="space-y-5 text-left">
      ...
      <div className="relative">
        ...
        <div className="bg-zinc-900/50 border border-zinc-800/80 rounded-3xl p-6 backdrop-blur-sm shadow-lg">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex items-center gap-3.5">
              <div className="relative h-11 w-11 rounded-2xl bg-gradient-to-br from-violet-600 to-indigo-600 flex items-center justify-center shadow-lg shadow-indigo-500/20 shrink-0">
                ...
                <div className="absolute -top-0.5 -right-0.5 h-2.5 w-2.5 rounded-full bg-emerald-400 border-2 border-zinc-900 animate-pulse" />
              </div>
              <div>
            ...
            <div className="flex items-center gap-3 shrink-0">
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold text-zinc-400 hover:text-indigo-400 border border-zinc-800 hover:border-indigo-500/30 bg-zinc-950 hover:bg-indigo-500/5 transition-all disabled:opacity-40 cursor-pointer"
          ...
          {summary && (
            <div className="mt-4 flex flex-wrap gap-3">
              <div className="flex items-center gap-1.5 text-xs text-zinc-500">
                ...
                <span className="font-semibold text-zinc-400">{summary.totalPosts}</span> posts analyzed
              </div>
              {summary.avgEngagementRate > 0 && (
                <div className="flex items-center gap-1.5 text-xs text-zinc-500">
                  <TrendingUp className="h-3.5 w-3.5 text-emerald-400" />
                  <span className="font-semibold text-emerald-400">{summary.avgEngagementRate}%</span> avg engagement
                </div>
              )}
      ...
      {!hasAnalyticsData && !loading && !data && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="bg-zinc-900/40 border border-zinc-800/60 rounded-2xl p-8 text-center space-y-3"
        >
          ...
          <p className="text-sm font-semibold text-zinc-400">No analytics data to analyze yet.</p>
          <p className="text-xs text-zinc-600 max-w-xs mx-auto">Sync your social media data first, then AI Suggestions will analyze your real performance and generate personalized recommendations.</p>
        </motion.div>
      )}
      ...
      {/* ── Error state ── */}
      {error && !loading && (
        <div className="bg-rose-500/8 border border-rose-500/20 rounded-2xl p-4 flex items-center gap-3 text-sm text-rose-400">
      ...
      {data?.llmUnavailable && !loading && (
        <div className="bg-amber-500/8 border border-amber-500/20 rounded-2xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-sm font-semibold text-amber-400">AI Engine Unavailable</p>
        ...
        >
          ...
          {s.performanceSummary && (
            <div className="bg-gradient-to-br from-indigo-600/8 via-violet-600/5 to-transparent border border-indigo-500/20 rounded-2xl p-5">
              ...
              <p className="text-sm text-zinc-200 leading-relaxed">{s.performanceSummary}</p>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
            ...
            {s.growthOpportunities?.length > 0 && (
              <SectionCard icon={TrendingUp} title="Growth Opportunities" iconColor="text-emerald-400">
                <div className="space-y-3 mt-2">
                  {s.growthOpportunities.map((opp, i) => (
                    <div key={i} className="flex gap-3">
                      ...
                      <div>
                        <p className="text-xs font-bold text-zinc-200">{opp.title}</p>
                        <p className="text-xs text-zinc-500 mt-0.5">{opp.description}</p>
                        {opp.metric && (
                          <span className="inline-block mt-1 text-[10px] font-bold text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-md">
                            📊 {opp.metric}
                          </span>
                        )}
                      </div>
            ...
            {s.contentRecommendations?.length > 0 && (
              <SectionCard icon={FileText} title="Content Recommendations" iconColor="text-sky-400">
                <div className="space-y-3 mt-2">
                    ...
                    <div key={i} className="flex gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-1.5 mb-0.5">
                          <span className="text-[10px] font-bold text-sky-400/80 uppercase tracking-wider">{rec.type}</span>
                        </div>
                        <p className="text-xs font-semibold text-zinc-200">{rec.suggestion}</p>
                        {rec.reason && <p className="text-xs text-zinc-500 mt-0.5">{rec.reason}</p>}
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {s.captionRecommendations && (
              <SectionCard icon={MessageSquare} title="Caption Recommendations" iconColor="text-violet-400">
                <div className="space-y-3 mt-2">
                  <div className="flex flex-wrap gap-2">
                    {s.captionRecommendations.length && (
                      <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-violet-500/15 text-violet-400">
                        Length: {s.captionRecommendations.length}
                      </span>
                    )}
                    {s.captionRecommendations.style && (
                      <span className="text-[10px] font-bold px-2.5 py-1 rounded-full bg-zinc-700/50 text-zinc-300">
                        Style: {s.captionRecommendations.style}
                      </span>
                    )}
                  </div>
                  {s.captionRecommendations.toneAdvice && (
                    <p className="text-xs text-zinc-400">{s.captionRecommendations.toneAdvice}</p>
                  )}
                  {s.captionRecommendations.hooks?.length > 0 && (
                    <div className="space-y-1.5">
                      <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider">Hook Examples</p>
                      {s.captionRecommendations.hooks.map((hook, i) => (
                        <div key={i} className="bg-zinc-800/40 rounded-lg px-3 py-2 text-xs text-zinc-300 italic border border-zinc-700/40">
                          "{hook}"
                        </div>
                      ))}
                    </div>
            ...
            {s.hashtagRecommendations && (
              <SectionCard icon={Hash} title="Hashtag Strategy" iconColor="text-fuchsia-400">
                <div className="space-y-3 mt-2">
                  {s.hashtagRecommendations.strategy && (
                    <p className="text-xs text-zinc-400">{s.hashtagRecommendations.strategy}</p>
                  )}
                  {s.hashtagRecommendations.suggestedHashtags?.length > 0 && (
                    <div className="flex flex-wrap gap-1.5">
                      {s.hashtagRecommendations.suggestedHashtags.map((tag, i) => (
                        <span key={i} className="text-xs font-bold px-2.5 py-1 rounded-full bg-fuchsia-500/10 text-fuchsia-400 border border-fuchsia-500/20">
            ...
            {s.audienceInsights?.length > 0 && (
              <SectionCard icon={Users} title="Audience Insights" iconColor="text-amber-400">
                <div className="space-y-3 mt-2">
                  {s.audienceInsights.map((insight, i) => (
                    <div key={i} className="border-l-2 border-amber-500/30 pl-3 space-y-0.5">
                      <p className="text-xs font-semibold text-zinc-200">{insight.insight}</p>
                      {insight.actionable && (
                        <p className="text-xs text-amber-400/80">→ {insight.actionable}</p>
                      )}
                    </div>
                  ))}
                </div>
            ...
            {/* Posting Strategy */}
            {s.postingStrategy && (
              <SectionCard icon={Calendar} title="Posting Strategy" iconColor="text-rose-400">
                <div className="space-y-2.5 mt-2">
                  {s.postingStrategy.bestDays?.length > 0 && (
                    <div>
                      <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Best Days</p>
                      <div className="flex flex-wrap gap-1.5">
                        {s.postingStrategy.bestDays.map((day, i) => (
                          <span key={i} className="text-xs font-bold px-2.5 py-1 rounded-full bg-rose-500/10 text-rose-400 border border-rose-500/20">
                            {day}
                          </span>
                        ))}
                      </div>
                  ...
                  {s.postingStrategy.bestHours && (
                    <div>
                      <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Best Time</p>
                      <p className="text-xs font-semibold text-zinc-200">{s.postingStrategy.bestHours}</p>
                    </div>
                  )}
                  {s.postingStrategy.frequency && (
                    <div>
                      <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-wider mb-1">Frequency</p>
                      <p className="text-xs font-semibold text-zinc-200">{s.postingStrategy.frequency}</p>
                    </div>
                  )}
                  {s.postingStrategy.rationale && (
                    <p className="text-xs text-zinc-500 leading-relaxed border-t border-zinc-800 pt-2">{s.postingStrategy.rationale}</p>
                  )}
                </div>
              </SectionCard>
            )}
          ...
          {s.priorityActions?.length > 0 && (
            <SectionCard icon={Target} title="Priority Action Items" iconColor="text-indigo-400" defaultOpen={true}>
              <div className="space-y-3 mt-2">
                {s.priorityActions.map((action, i) => (
                  <motion.div
                    ...
                    className="flex items-start gap-3 bg-zinc-800/30 rounded-xl p-3.5 border border-zinc-800/50 hover:border-zinc-700/60 transition-colors"
                  >
                    <div className="flex items-center gap-2 shrink-0 pt-0.5">
                      <span className="text-xs font-black text-zinc-500 w-4 text-center">{i + 1}</span>
          ...
          {learningEnabled && (
            <div className="flex items-center justify-center gap-2 text-[10px] text-zinc-600 py-1">
